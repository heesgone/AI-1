...
# 데이터 증강 (Augmentation) 기법을 적용하여 모델의 일반화 성능을 향상 (예: 회전, 이동, 좌우반전 등)
...
train_transform = transforms.Compose([
    ...
    transforms.RandomAffine(degrees=0, translate=(0.1, 0.1)), # 상하좌우 미세 이동
    transforms.ColorJitter(brightness=0.2, contrast=0.2),    # 밝기, 대비 조절
    transforms.ToTensor(),
    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]) # 정규화(성능 향상에 필수!)
])

# 테스트/검증용은 증강 없이 Resize와 Normalize만!
...
#########################################
# Augmentation Sample Display
#########################################
print("\n==== 증강된 이미지 샘플 추출 중... ====")

# 시각화를 위해 텐서를 다시 이미지로 변환하는 함수
def denormalize(tensor):
    # Normalize 시 사용했던 값 (0.485, 0.456, 0.406), (0.229, 0.224, 0.225)
    mean = torch.tensor([0.485, 0.456, 0.406]).view(3, 1, 1)
    std = torch.tensor([0.229, 0.224, 0.225]).view(3, 1, 1)
    return tensor * std + mean

# train_loader에서 8개의 이미지만 가져오기
sample_batch_images, sample_batch_labels = next(iter(train_loader))

plt.figure(figsize=(15, 8))
for i in range(8): # 8개 샘플 시각화
    plt.subplot(2, 4, i+1)
    
    # 텐서 -> 넘파이 변환 (이미지 채널 순서 변경: C,H,W -> H,W,C)
    img = denormalize(sample_batch_images[i])
    img = img.permute(1, 2, 0).numpy()
    img = np.clip(img, 0, 1) # 이미지 값 범위를 0~1로 제한
    
    plt.imshow(img)
    plt.title(f"Augmented: {class_names[sample_batch_labels[i]]}")
    plt.axis('off')

plt.tight_layout()
plt.savefig('augmented_samples_dev6.png')
print("=== 증강 샘플 이미지(augmented_samples_dev6.png) 저장 완료! ===\n")
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.first_pass = True  # 데이터 흐름 확인을 위한 플래그
...
optimizer = optim.Adam(cnn.parameters(), lr=0.001, weight_decay=1e-4) # L2 정규화 (weight decay)
...
plt.tight_layout()
plt.savefig('training_curves_dev6.png')
print("\n=== 학습 그래프(training_curves_dev6.png) 저장 완료 ===")
...
def save_feature_maps(feature_map, layer_name, num_kernels=16):
    ...
    plt.tight_layout()
    filename = f'feature_map_{layer_name}_dev6.png'
