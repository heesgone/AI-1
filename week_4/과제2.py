...
#########################################
# 데이터셋 설명
# Food-11 데이터셋은 11가지 음식 카테고리를 포함하는 이미지로 구성
# 11가지의 음식 카테고리 중에서 5개의 카테고리만 선별하여 분류 (chicken_curry, french_fries, hamburger, pizza, sushi) 
# 각 클래스별 학습 샘플 수는 900장, 테스트 샘플 수는 100장으로 구성 (총 5,000장)
# 학습용 데이터는 8:2 비율로 클래스별로 균등하게 나눠 train과 validation으로 분할 (train: 720장, validation: 180장)
# ImageFolder를 사용하여 데이터셋을 로드
# 이미지별 사이즈가 달라서, CNN 모델에 입력하기 전에 모든 이미지를 256x256으로 Resize하는 전처리 과정 추가
#########################################
...
#labels_map = {v:k for k, v in train_folder.class_to_idx.items()}
...
# plt.savefig('food_samples.png')
# print("--- 샘플 이미지(food_samples.png) 저장 완료 ---")

#########################################
# 클래스별 샘플 개수 출력 (분포 유지 확인을 위함)
#########################################
from collections import Counter

def class_distribution(dataset, folder_obj, name):
    # Subset에서 원래 Dataset의 클래스 인덱스를 가져와 클래스별 개수 계산
    ...
    for idx in indices:
        class_idx = folder_obj.targets[idx]
        class_counts[class_idx] += 1
    
    # 결과 출력
    print(f"\n==== {name} 클래스별 샘플 개수 ====")
    # 정렬해서 보여주기 위해 folder_obj.classes를 기준으로 루프
    for idx, class_name in enumerate(folder_obj.classes):
        print(f"{class_name:<15}: {class_counts[idx]} samples")

class_distribution(train_dataset, train_folder, "Train")
class_distribution(val_dataset, train_folder, "Validation")
class_distribution(test_folder, test_folder, "Test")
...
print("\n=== 학습 그래프(training_curves_baseline.png) 저장 완료 ===")
...
def save_feature_maps(feature_map, layer_name, num_kernels=16):
    ...
    for i in range(num_kernels):
        plt.subplot(4, 4, i+1)
        plt.imshow(f_map[i], cmap='viridis') # 'gray' 대신 색감이 있는 'viridis' 추천!
        plt.axis('off')
        plt.title(f'Kernel {i}')
    
    plt.tight_layout()
    filename = f'feature_map_{layer_name}.png'
